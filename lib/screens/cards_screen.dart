...
class CardsScreenState extends State<CardsScreen> {
  ...
  Future<void> initializePage() async {
    try {
      goToPage = Provider.of<DivineNames>(
        context,
        listen: false,
      ).lastNameViewed;
    } catch (e) {
      debugPrint('error caught in initializePage');
      goToPage = 1;
    }
  }
...
class NameCards extends StatefulWidget {
  ...
  const NameCards({super.key, required this.goToPage, this.newSession = true});
...
class _NameCardsState extends State<NameCards> {
  ...
  void didChangeDependencies() {
    ...
    debugPrint('${mediaQuery.size.width} x ${mediaQuery.size.height}');
